...
DEFAULT_STREAM_URL = "http://localhost:8889/rudderpi"
...
def _map_output_channel(logical_ch: int) -> int:
    """
    Allows swapping outputs without rewiring.
    logical 0/1 are "left/right" after mixing.
    """
    if not SWAP_OUTPUTS:
        return logical_ch
...
def pwm_worker_forever() -> None:
    ...
    while True:
        ...
        # Detect forward<->reverse switch on GAS and apply neutral hold
        ...
        if now < _hold_until_ts:
            # During hold, force neutral (0) regardless of requested target
        ...
        # Apply to outputs (with optional channel swap)
